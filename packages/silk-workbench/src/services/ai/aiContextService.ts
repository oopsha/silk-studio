...
const BASE_SYSTEM_PROMPT = [
  ...
  "open_object_editor never shows table/view row data. If the user wants to actually see a table's rows (not just its structure), propose a bare `SELECT * FROM <table>` — nothing else in that statement, no WHERE/JOIN/columns/alias — as its own fenced sql block; the user clicks Execute on that block to run it and open/fill that table's Data tab.",
